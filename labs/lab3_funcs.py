...
def lettergrade(pct):
    ...
    # i understand that we're expected to use if/else / conditionals, but i wanted to have fun with it
    grades = ['F', 'F', 'F', 'F', 'F', 'D', 'C', 'B', 'A', 'A', 'A']

    if pct > 100 or pct < 0: # check out of bounds condition
        return None

    return grades[pct // 10] # 'pct // 10' returns the digit in the tens column, so we can use it as an index into the array of possible grades.
...
def inversions(items):
    ...
    if len(items) > 0:
        # only nested it this way to ensure no out-of-bounds errors when accessing items[0]
        if not (isinstance(items[0], int) or isinstance(items[0], str)):
            return -1

    if len(items) == 3:
        return (items[0] > items[1]) + (items[0] > items[2]) + (items[1] > items[2])
...
def increasing(items, strict):
    ...
    if len(items) != 3 or type(strict) != type(True):
        return 'invalid input'

    if strict:
        return items[0] < items[1] and items[1] < items[2]
